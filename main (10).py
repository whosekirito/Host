...
def detect_and_install_dependencies(file_path, is_premium=False):
    ...
    try:
        ...
        # Enhanced dependency mapping with more packages
        dependency_map = {
            'requests': 'requests',
            'flask': 'flask',
            'django': 'django',
        ...
        # Find import statements with better regex
        import re
        # Match various import patterns
        imports = re.findall(r'(?:^from\s+([a-zA-Z_][a-zA-Z0-9_]*)|^import\s+([a-zA-Z_][a-zA-Z0-9_]*))(?:\s|$|\.)', content, re.MULTILINE)
        detected_deps = set()
        ...
        if detected_deps:
            ...
            try:
                ...
                try:
                    result = subprocess.run(["uv", "add"] + deps_list, 
                                           check=True, capture_output=True, text=True, timeout=60)
                    return True, f"✅ Auto-installed with uv: {', '.join(deps_list)}"
                except (subprocess.CalledProcessError, FileNotFoundError, subprocess.TimeoutExpired):
                    result = subprocess.run(["python3", "-m", "pip", "install"] + deps_list, 
                                           check=True, capture_output=True, text=True, timeout=60)
                    return True, f"✅ Auto-installed with pip: {', '.join(deps_list)}"
            except subprocess.CalledProcessError as e:
                error_msg = e.stderr if e.stderr else str(e)
                return False, f"❌ Failed to auto-install dependencies: {error_msg}"
    ...
    if data == "create_bot":
        ...
        await query.edit_message_caption(
            caption=f"💳 **Your Subscription**\n\n"
                   ...
                   f"{'Upgrade to Premium for more bots and auto dependency installation!' if sub_type == 'free' else 'Thank you for being Premium!'}",
            reply_markup=reply_markup,
            parse_mode='Markdown'
        )
        ...
        keyboard = [
            [InlineKeyboardButton("💸 Pay ₹799", url="https://t.me/whosekirito")],
...
4. Upload requirements.txt file (or use auto dependency for Premium)
...
🔄 Auto-restart
⚙️ Easy management
🔧 Auto dependency (Premium)

**Limits:**
🆓 Free: 1 bot
💎 Premium: 15 bots + auto dependencies (₹799/month)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == 'waiting_name':
        ...
        # Check if user is premium for auto dependency installation
        sub_type, _, _ = get_user_subscription(user_id)

        if sub_type == 'premium':
            set_creation_state(user_id, 'auto_dependencies', bot_name=bot_name, main_file_path=main_file_path)

            keyboard = [
                [InlineKeyboardButton("✅ Auto Install Dependencies", callback_data="auto_deps"),
                 InlineKeyboardButton("📄 Upload requirements.txt", callback_data="manual_deps")],
                [InlineKeyboardButton("❌ Cancel", callback_data="cancel_creation")]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)

            await update.message.reply_text(
                "✅ Main file uploaded!\n\n💎 **Premium Feature:** You can choose auto dependency installation or upload requirements.txt manually:",
                reply_markup=reply_markup,
                parse_mode='Markdown'
            )
        else:
        ...
        await process_bot_creation(update, context, user_id, bot_name, main_file_path, requirements_path, manual=True)

# Handle auto dependency installation callback
async def handle_auto_deps_callback(update, context):
    query = update.callback_query
    user_id = query.from_user.id
    data = query.data
    ...
    if data == "auto_deps" and state == 'auto_dependencies':
        await query.answer("🔄 Starting auto dependency installation...", show_alert=True)
        await process_bot_creation(update, context, user_id, bot_name, main_file_path, None, manual=False)

    elif data == "manual_deps" and state == 'auto_dependencies':
        set_creation_state(user_id, 'waiting_requirements', bot_name=bot_name, main_file_path=main_file_path)
...
async def process_bot_creation(update, context, user_id, bot_name, main_file_path, requirements_path, manual=True):
    """Process bot creation with either manual requirements or auto dependency installation"""
    ...
    if hasattr(update, 'callback_query'):
        ...
        edit_func = lambda text, **kwargs: message.reply_text(text, **kwargs)

    status_msg = await edit_func("📦 Installing dependencies... Please wait...")

    # Install dependencies
    if manual and requirements_path:
        # Manual requirements.txt installation
        success, message_text = install_requirements(requirements_path)
    else:
        # Auto dependency detection for premium users
        sub_type, _, _ = get_user_subscription(user_id)
        success, message_text = detect_and_install_dependencies(main_file_path, is_premium=(sub_type == 'premium'))
...
async def run_telegram_bot():
    ...
    # Enhanced callback query handler to handle auto dependency callbacks
    async def enhanced_callback_handler(update, context):
        if update.callback_query.data in ["auto_deps", "manual_deps"]:
            await handle_auto_deps_callback(update, context)
        else:
            await button_callback(update, context)
